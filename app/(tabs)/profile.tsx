...
const settingsMenuItems = [
  { id: '1', title: 'Account', icon: 'person-outline' as const, route: '/settings/account' as const },
  { id: '2', title: 'App Preferences', icon: 'globe-outline' as const, route: '/settings/preferences' as const },
  { id: '3', title: 'Privacy & Security', icon: 'shield-outline' as const, route: null },
  { id: '4', title: 'Support & Info', icon: 'help-circle-outline' as const, route: '/settings/support' as const },
];
...
export default function ProfileScreen() {
  ...
  const [collectionSortOption, setCollectionSortOption] = useState<SortOption>('newest');
  const [favoritesSortOption, setFavoritesSortOption] = useState<SortOption>('newest');
  ...
  const [addToFolderItemId, setAddToFolderItemId] = useState<string | null>(null);
  const [newFolderName, setNewFolderName] = useState('');
  const [newFolderColor, setNewFolderColor] = useState(FOLDER_COLORS[0].hex);
  const [isSavingFolder, setIsSavingFolder] = useState(false);

  // Auth and data hooks
  const { user, token, isAuthenticated, logout, refreshUser, isLoading: authLoading } = useAuth();
  const { favorites, isLoading: favoritesLoading, removeFromFavorites, refresh: refreshFavorites } = useFavorites();
  ...
  const currentSortOption = activeTab === 'collection' ? collectionSortOption : favoritesSortOption;
  const setCurrentSortOption = activeTab === 'collection' ? setCollectionSortOption : setFavoritesSortOption;
  ...
  const handleProfilePicturePress = () => {
    ...
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        {
          options: ['Cancel', 'Take Photo', 'Choose from Library', ...(user?.profile_picture ? ['Remove Photo'] : [])],
      ...
      Alert.alert(
        ...
        [
          ...
          ...(user?.profile_picture ? [{ text: 'Remove Photo', style: 'destructive' as const, onPress: handleRemovePhoto }] : []),
        ]
      );
    }
  };
  ...
  const handleTakePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        await uploadProfilePicture(result.assets[0].uri, result.assets[0].mimeType);
      }
    } catch (error) {
  ...
  const uploadProfilePicture = async (uri: string, mimeType?: string | null) => {
    if (!token) return;

    setIsUploadingPhoto(true);
    try {
      await authService.uploadProfilePicture(token, uri, mimeType || 'image/jpeg');
      await refreshUser();
  ...
  const handleDeleteFavorite = async (speciesId: string) => {
    console.log('[Profile] handleDeleteFavorite called with speciesId:', speciesId);
    setDeletingItemId(speciesId);
    try {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Deleted', `Folder deleted. ${result.items_affected} items kept in collection.`);
            } catch (error: any) {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Deleted', `Folder and ${result.items_affected} items deleted.`);
            } catch (error: any) {
              Alert.alert('Error', error.message || 'Failed to delete folder');
            }
          }
  ...
  const handleToggleItemFolder = async (speciesId: string, folderId: string, isCurrentlyIn: boolean) => {
        ...
        {isAuthenticated && (
          <View className="px-5 mt-8">
            ...
            >
              ...
              <Text className="ml-2 text-base font-medium text-red-500">Log Out</Text>
            </TouchableOpacity>
          </View>
        )}
      </SafeAreaView>
      ...
      >
        <View className="flex-1 justify-end">
          <View className="bg-white rounded-t-3xl">
            ...
            >
              <Text className={`text-base ${currentSortOption === 'newest' ? 'text-[#22A45D] font-semibold' : 'text-gray-700'}`}>
                Newest First
              </Text>
            ...
            >
              <Text className={`text-base ${currentSortOption === 'oldest' ? 'text-[#22A45D] font-semibold' : 'text-gray-700'}`}>
                Oldest First
              </Text>
            ...
            >
              <Text className={`text-base ${currentSortOption === 'name-asc' ? 'text-[#22A45D] font-semibold' : 'text-gray-700'}`}>
                Name (A-Z)
              </Text>
            ...
            >
              <Text className={`text-base ${currentSortOption === 'name-desc' ? 'text-[#22A45D] font-semibold' : 'text-gray-700'}`}>
                Name (Z-A)
              </Text>
    ...
    return (
      <SafeAreaView className="flex-1 bg-white">
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="text-2xl font-bold text-gray-800 mb-2">Welcome to Antify</Text>
          ...
          >
            <Text className="text-[#0A9D5C] font-semibold text-center text-lg">Create Account</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
  ...
  const addToFolderItem = collection.find(c => c.species_id === addToFolderItemId);
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        <View className="flex-1 bg-black/30 justify-center items-center">
          <View className="bg-white rounded-2xl mx-6 w-[90%] max-w-[340px]">
            <View className="p-6">
              <Text className="text-xl font-bold text-center text-gray-800 mb-4">New Folder</Text>
              
              {/* Folder Name Input */}
              <Text className="text-sm font-medium text-gray-600 mb-2">Folder Name</Text>
              <TextInput
              ...
              {/* Color Picker */}
              <Text className="text-sm font-medium text-gray-600 mb-2">Color</Text>
                  ...
                  >
                    <View 
                      className={`w-10 h-10 rounded-full items-center justify-center ${
                        newFolderColor === color.hex ? 'border-2 border-gray-800' : ''
                      }`}
              ...
              <View className="flex-row">
                ...
                <TouchableOpacity
                  ...
                  style={{ opacity: isSavingFolder || !newFolderName.trim() ? 0.5 : 1 }}
                >
                  {isSavingFolder ? (
                    <ActivityIndicator size="small" color="#FFFFFF" />
                  ) : (
      ...
      >
        <View className="flex-1 bg-black/30 justify-center items-center">
          <View className="bg-white rounded-2xl mx-6 w-[90%] max-w-[340px]">
            <View className="p-6">
              <Text className="text-xl font-bold text-center text-gray-800 mb-4">Edit Folder</Text>
      ...
      >
        <View className="flex-1 bg-black/30 justify-center items-center">
          <View className="bg-white rounded-2xl mx-6 w-[90%] max-w-[340px]">
            <View className="p-6">
              <Text className="text-xl font-bold text-center text-gray-800 mb-2">Add to Folder</Text>
              ...
              {folders.length === 0 ? (
                <View className="items-center py-6">
                  ...
                  <Text className="text-gray-500 mt-2 text-center">No folders yet</Text>
                  <TouchableOpacity
                  ...
                  >
                    <Text className="text-white font-medium">Create Folder</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <View className="mb-4">
                  {folders.map((folder) => {
                    const isInFolder = addToFolderItemFolderIds.includes(folder.id);
                    return (
                      <TouchableOpacity
                        key={folder.id}
                        ...
                        onPress={() => {
                          if (addToFolderItemId) {
                            handleToggleItemFolder(addToFolderItemId, folder.id, isInFolder);
                          }
                        }}
                      >
                        <View 
              ...
              >
                <Text className="text-center text-gray-600 font-medium">Done</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
              ...
              sortedCollection.length > 0 ? (
                      ...
                      {isEditMode && (
                        <>
                          ...
                          <TouchableOpacity
                            ...
                            style={{ zIndex: 10, left: index % 2 === 0 ? 2 : 8 }}
                            onPress={() => handleOpenAddToFolder(item.species_id)}
                          >
                          ...
                          <TouchableOpacity
                            ...
                            style={{ zIndex: 10, right: index % 2 === 0 ? 8 : 2 }}
                            onPress={() => handleDeleteCollection(item.species_id)}
                ...
                <View className="items-center py-12">
                  ...
                  <Text className="mt-4 text-gray-500 text-center">
                    Your collection is empty.{'\n'}Start adding species you've found!
                  </Text>
                  <TouchableOpacity
                    className="mt-6 bg-[#0A9D5C] rounded-full px-6 py-3"
                    onPress={() => router.push('/(tabs)/explore')}
                  >
                    <Text className="text-white font-semibold">Explore Species</Text>
                  </TouchableOpacity>
                </View>
              )
            ) : (
