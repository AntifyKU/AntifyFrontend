...
export function useProvinceSpecies(province: string | null): UseProvinceSpeciesReturn {
    ...
    useEffect(() => {
        ...
        const fetchProvinceSpecies = async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    setError(err instanceof Error ? err : new Error('Failed to fetch province species'));
