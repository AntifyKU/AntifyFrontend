...
export function useCollection(): UseCollectionResult {
  ...
  const fetchCollection = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[useCollection] Fetched collection, count:",
  ...
  }, [isAuthenticated, token, fetchCollection]);
  ...
  const getCollectionItem = useCallback(
    (speciesId: string): CollectionItem | undefined => {
  ...
  const addToCollection = useCallback(
    async (
      ...
      try {
        ...
        console.log(
          "[useCollection] API returned newItem:",
        ...
        setCollection((prev) => {
          ...
          console.log(
            "[useCollection] Updated collection length:",
            updated.length,
          );
          return updated;
        });
        console.log(
          "[useCollection] Added to collection:",
          newItem.id,
          "species_name:",
          newItem.species_name,
        );
        return newItem;
      } catch (err: any) {
  ...
  const removeFromCollection = useCallback(
    async (speciesId: string) => {
      ...
      if (!item) {
        console.warn(
          "[useCollection] Item not found in collection:",
          speciesId,
        );
        return;
      }
      ...
      console.log(
        "[useCollection] Removing from collection:",
        item.id,
        "for species:",
        speciesId,
      );
  ...
  const addItemToFolders = useCallback(
    async (speciesId: string, folderIds: string[]) => {
      if (!token) {
      ...
      if (!item) {
        console.warn(
          "[useCollection] Item not found for folder assignment:",
          speciesId,
        );
        return;
      }
      ...
      try {
        const result = await foldersService.addItemToFolders(
          token,
          item.id,
          folderIds,
        );
        ...
        console.log(
          "[useCollection] Added item to folders:",
          result.folder_ids,
        );
      } catch (err: any) {
  ...
  const removeItemFromFolder = useCallback(
    async (speciesId: string, folderId: string) => {
      if (!token) {
        throw new Error("You must be logged in to manage folders");
      }
      ...
      if (!item) {
        console.warn(
          "[useCollection] Item not found for folder removal:",
          speciesId,
        );
        return;
      }
      ...
      try {
        const result = await foldersService.removeItemFromFolder(
          token,
          item.id,
          folderId,
        );
  ...
  const setItemFolders = useCallback(
    async (speciesId: string, folderIds: string[]) => {
      ...
      if (!item) {
        console.warn(
          "[useCollection] Item not found for folder update:",
          speciesId,
        );
        return;
      }
      ...
      try {
        const result = await foldersService.setItemFolders(
          token,
          item.id,
          folderIds,
        );
  ...
  const getItemsByFolder = useCallback(
    (folderId: string | null): CollectionItem[] => {
      if (folderId === null) {
        return collection;
      }
