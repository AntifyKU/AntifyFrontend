...
export function useFolders(): UseFoldersResult {
  ...
  }, [isAuthenticated, token, fetchFolders]);
  ...
  const updateFolder = useCallback(
    async (id: string, data: UpdateFolderRequest): Promise<void> => {
      if (!token) {
        throw new Error("You must be logged in to update folders");
      }

      try {
        const updatedFolder = await foldersService.updateFolder(
          token,
          id,
          data,
        );
